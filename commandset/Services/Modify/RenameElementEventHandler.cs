...
{
    public class RenameElementEventHandler : WaitableEventHandlerBase, IExternalEventHandler, IWaitableExternalEventHandler
    {
            ...
            try
            {
                ...
                using (var trans = new Transaction(Doc, "Rename Element"))
                {
                    ...
                    foreach (Parameter param in element.Parameters)
                    {
                        ...
                        if (def != null && RevitUiTerms.Matches(RevitUiTerms.NameParameter, def.Name))
                        {
                            if (!param.IsReadOnly)
                            {
                                param.Set(NewName);
