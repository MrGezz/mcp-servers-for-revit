...
{
    public class SetViewRangeEventHandler : WaitableEventHandlerBase, IExternalEventHandler, IWaitableExternalEventHandler
    {
        ...
        public void Execute(UIApplication uiapp)
        {
            ...
            try
            {
                using (Transaction trans = new Transaction(doc, "Set View Range"))
                {
                    ...
                    ViewPlan viewPlan = doc.GetElement(ElementIdFactory.Create(ViewId)) as ViewPlan;
                    if (viewPlan == null)
                    {
                    ...
                    // PlanViewRange, ViewPlan.GetViewRange, ViewPlan.SetViewRange,
                    // PlanViewRange.SetLevelId and PlanViewRange.SetOffset are all
                    // IDENTICAL on 2022-2027. No version guard is required.
                    PlanViewRange viewRange = viewPlan.GetViewRange();

                    if (TopLevelId.HasValue)
                    {
                        ElementId levelId = ElementIdFactory.Create(TopLevelId.Value);
