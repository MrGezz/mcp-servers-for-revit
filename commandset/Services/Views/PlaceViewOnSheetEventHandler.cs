            ...
            try
            {
                ...
                foreach (var info in CreatedInfo)
                {
                    using (Transaction trans = new Transaction(doc, "Place Viewport"))
                    {
                        ...
                        ElementId sheetId = ElementIdFactory.Create(info.SheetId);
                        ...
                        ElementId viewId = ElementIdFactory.Create(info.ViewId);
                        ...
                        if (viewport != null)
                        {
                            ...
                            {
                                Element vpTypeElem = doc.GetElement(ElementIdFactory.Create(info.ViewportTypeId));
                                if (vpTypeElem != null)
                                {
                            ...
                            if (info.ScaleOverride > 0)
                            {
                                ...
                                {
                                    _warnings.Add($"Could not override scale to {info.ScaleOverride}: {ex.Message}");
                                }
                            }
                            ...
                            if (info.Rotation > 0)
                            {
                                try
                                {
                                    viewport.Rotation = (ViewportRotation)(int)info.Rotation;
                                }
                                catch (Exception ex)
                                {
                                    _warnings.Add($"Could not set rotation to {info.Rotation}: {ex.Message}");
                                }
                            }
