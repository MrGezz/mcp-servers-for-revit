        ...
        public void Execute(UIApplication uiapp)
        {
            ...
            try
            {
                using (Transaction trans = new Transaction(doc, "Create Callout"))
                {
                    ...
                    View hostView = doc.GetElement(ElementIdFactory.Create(HostViewId)) as View;
                    if (hostView == null)
                    {
                    ...
                    // ViewSection.CreateCallout returns View - the concrete type depends on the
                    // ViewFamilyType, so it is not always a ViewSection.
                    View callout = VersionCompat.CreateCallout(doc, hostView.Id, vft.Id, bbox);
